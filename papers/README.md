...
|-------|-------|------------------|------------------------------|
| Deng et al. (2508.02180) | Quantized Test-Time Adaptation | ZOA: gradient-free TTA via zeroth-order optimization (SPSA), using only 2 forward passes vs. 28 for prior SOTA (FOA); adds continual domain memory (DRL) to compensate for noisy gradient estimates; +5% over FOA on ImageNet-C | Assumes detectable domain shift; limited to ViT/ResNet/Mamba vision architectures; no reasoning tasks evaluated; no wall-clock benchmarks on real edge hardware; hand-tuned domain similarity threshold |
| Feng et al. (2504.10903) | Survey: Efficient Reasoning Models | Organizes the efficiency landscape into 3 orthogonal axes — Shorter (CoT compression), Smaller (distillation/quantization/pruning), Faster (decoding strategies); key finding: pruning generally harms reasoning unlike standard LLMs; small models (Qwen2.5 1.5B) can reason well via RL; LIMO shows 817 samples can suffice | Safety-efficiency tension underexplored (shorter CoTs may skip safety self-correction); sustainability metrics absent; survey scope means no new empirical results |
| Wu et al. (2510.02611) | Temperature Scaling in Test-Time Scaling | Shows single-temperature TTS plateaus at large K; different temperatures solve different problem subsets; scaling across temperatures T=0.0–1.2 gives +7.3 pts avg over Qwen3 (0.6B–8B) and 5 benchmarks; base models match RL-trained models on AIME 2025 without post-training; multi-temperature voting cuts compute 26–54% | Compute scales linearly with number of temperatures; RL comparison narrow (30 AIME problems, 1 model); "skip T=0.1–0.3" heuristic empirically derived on Qwen3, may not generalize; no explanation of *why* specific temperatures unlock specific problems |
| Liu et al. (2504.04823) | Quantization of Reasoning Models | First systematic study across 7 models (1.5B–70B), 12 algorithms, 5 benchmarks; W4A16 is effectively lossless (≤1% drop even on AIME); hard tasks degrade 4× more at W4A4; model origin (distilled vs. RL-trained) matters more than architecture; quantized models don't generate longer CoTs when accuracy drops | Heavy math bias (AIME/GSM8K); no fine-grained error localization in CoT (where does reasoning break?); static quantization only; no safety/calibration analysis |
| Rakka et al. (2510.16805) | Mixed-Precision Quantization Survey | Formal taxonomy distinguishing true MP (varying bitwidths *within* layers) from misnomers; 4-bit avg precision preserves accuracy (<2% perplexity Δ); 2-bit fails catastrophically (>55% Δ); MPW/MPW+UPA outperform MPW+MPA (activations harder to quantize than weights); search-based optimization infeasible at billion-param scale | No standardized benchmark suite; hardware support lags (dequantization overhead on GPUs); KV-cache quantization largely unexplored despite dominating long-context memory; energy-accuracy trade-off curves missing |
